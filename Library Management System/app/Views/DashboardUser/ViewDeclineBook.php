        ...
        <?php foreach ($users as $rent) { ?>
            <tr>
                ...
                <td class="d-flex gap-3">
                    <form action="/dashboardUser/retryRequestBook" method="post">
                        <input type="hidden" value="<?= $rent["idBook"] ?>" name="getIdBook">
                        <input type="hidden" value="<?= $rent["idRent"] ?>" name="getIdRent">
                        <button class="btn btn-success" type="submit" onclick="return confirm('Do you sure retry request this book ?')">Retry</button>
                    </form>

                    <form action="/dashboardUser/cancelRentBook" method="post">
                        <input type="hidden" value="<?= $rent["idBook"] ?>" name="getIdBook">
                        <button class="btn btn-warning" type="submit" onclick="return confirm('Do you sure cancel request this book ?')">Cancel Request</button>
                    </form>
                </td>
            </tr>
        <?php } ?>
