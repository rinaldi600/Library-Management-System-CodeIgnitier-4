        ...
        <?php if (count($users) <= 0) { ?>
            <td colspan="9">
                <div class="alert alert-danger" role="alert">
                    Data Not Found <a href="/dashboardAdmin/acceptUser">Back</a>
                </div>
            </td>
        <?php } ?>
        <?php foreach ($users as $rent) { ?>
            <tr>
                ...
                <td class="d-flex gap-3">
                    <form action="/dashboardAdmin/cancelAcceptRequest" method="post">
                        <input type="hidden" value="<?= $rent["idRent"] ?>" name="getIdRent">
                        <button class="btn btn-info" type="submit" onclick="return confirm('Do you sure cancel accept book ?')">Cancel Request</button>
                    </form>
                </td>
            </tr>
        <?php } ?>
