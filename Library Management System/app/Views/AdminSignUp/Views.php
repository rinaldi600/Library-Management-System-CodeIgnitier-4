...
<form action="/admin/getDataSignUp" method="post" enctype="multipart/form-data">
    <?php if (session()->getFlashdata('successSignUpAdmin')) { ?>
        <div class="alert alert-success position-absolute top-50 start-50 translate-middle text-center" role="alert">
            <?= session()->getFlashdata('successSignUpAdmin') ?> Klik <a class="text-decoration-none" href="/admin">Login</a>
        </div>
    <?php } ?>
    <div class="mb-3">
        <label for="idAdmin" class="form-label">ID Admin</label>
        <input type="text" class="form-control <?= session()->getFlashdata('idAdmin') ? 'is-invalid' : '' ?>" id="idAdmin" name="idAdmin" placeholder="Enter ID Admin....." value="<?= old('idAdmin') ? old('idAdmin') : '' ?>">
        <div id="emailHelp" class="form-text">We'll never share your ID Admin with anyone else.</div>
        <div id="validationServer03Feedback" class="invalid-feedback">
            <?= session()->getFlashdata('idAdmin') ?>
        </div>
    </div>
    <div class="mb-3">
